...
export default defineConfig([
  globalIgnores([
    'dist',
    // Chapter 30: the Capacitor native project is committed (see frontend/.gitignore
    // for why), but its Gradle build output is not source — it is a copy of a
    // Capacitor-authored runtime file (`native-bridge.js`) plus whatever plugins
    // generate, regenerated by every `gradlew` invocation. Linting it fails for a
    // reason that has nothing to do with this app: it ships its own
    // `eslint-disable` comments for a config this project doesn't have, so
    // `@typescript-eslint/no-unused-vars` reads as an unknown rule and errors.
    'android/**/build/**',
  ]),
  {
    files: ['**/*.{ts,tsx}'],
